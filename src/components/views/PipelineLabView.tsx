...
import { GraphExecution, PipelineNode } from '../../types';
...
export const PipelineLabView: React.FC = () => {
  const { agents, graphs, loadGraphs, createPipeline, deletePipeline, executeGraph, darkMode } = useStudioStore();
  ...
  const reset = () => { setId(pipelineId()); setName('Untitled pipeline'); setNodes(agents[0] ? [{ id: nodeId(), agent_id: agents[0].id }] : []); setLastRun(undefined); };
  ...
  const publish = async () => {
    ...
    setBusy(true);
    try {
      const graph = await createPipeline({ id: id.trim(), name: name.trim(), nodes });
  ...
  const run = async () => {
    ...
    setBusy(true);
    try {
      const result = await executeGraph(target, input);
      setLastRun(result);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid gap-6 xl:grid-cols-[300px_minmax(0,1fr)_300px]">
          <aside className={`h-fit rounded-2xl border p-5 space-y-4 ${surface}`}>
            ...
            <Field label="Pipeline ID"><input value={id} onChange={(event) => setId(event.target.value)} className={control} /></Field>
            <Field label="Display name"><input value={name} onChange={(event) => setName(event.target.value)} className={control} /></Field>
            ...
            <button onClick={publish} disabled={busy || !agents.length} className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-60 hover:bg-blue-700"><CheckCircleIcon className="w-4 h-4" />{busy ? 'Working…' : 'Publish pipeline'}</button>
          </aside>

          <section className={`min-h-[520px] rounded-2xl border p-5 ${surface}`}>
            <div className="flex items-center justify-between"><div><h3 className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Visual flow</h3><p className={`mt-0.5 text-xs ${muted}`}>Drag-free, keyboard-friendly authoring for a deterministic execution order.</p></div><button onClick={addNode} className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-semibold ${darkMode ? 'bg-gray-700 text-gray-100 hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}><PlusIcon className="w-4 h-4" /> Add agent</button></div>
